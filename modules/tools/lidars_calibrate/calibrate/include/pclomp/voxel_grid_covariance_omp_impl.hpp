...
#ifndef MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_VOXEL_GRID_COVARIANCE_OMP_IMPL_HPP_
#define MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_VOXEL_GRID_COVARIANCE_OMP_IMPL_HPP_
...
void pclomp::VoxelGridCovariance<PointT>::applyFilter(PointCloud& output) {
  ...
  if ((dx * dy * dz) > std::numeric_limits<int32_t>::max()) {
    PCL_WARN("[pcl::%s::applyFilter] Leaf size is too small. Integer indices would overflow.",
             getClassName().c_str());
    output.clear();
    return;
  }
  ...
  if (!filter_field_name_.empty()) {
    ...
    for (size_t cp = 0; cp < input_->points.size(); ++cp) {
      ...
      const uint8_t* pt_data = (const uint8_t*)&input_->points[cp];
      ...
      if (!downsample_all_data_) {
        ...
        if (rgba_index >= 0) {
          // fill r/g/b data
          int rgb;
          memcpy(&rgb, (const char*)&input_->points[cp] + rgba_index, sizeof(int));
    ...
    for (size_t cp = 0; cp < input_->points.size(); ++cp) {
      ...
      int ijk0 = static_cast<int>(floorf(input_->points[cp].x * inverse_leaf_size_[0]) -
                                  static_cast<float>(min_b_[0]));
      int ijk1 = static_cast<int>(floor(input_->points[cp].y * inverse_leaf_size_[1]) -
                                  static_cast<float>(min_b_[1]));
      int ijk2 = static_cast<int>(floor(input_->points[cp].z * inverse_leaf_size_[2]) -
  ...
  for (auto it = leaves_.begin(); it != leaves_.end(); ++it) {
    ...
    // and is added to the voxel centroids and output clouds.
...
#endif  // MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_VOXEL_GRID_COVARIANCE_OMP_IMPL_HPP_
