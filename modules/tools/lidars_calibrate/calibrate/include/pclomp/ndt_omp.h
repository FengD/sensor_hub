...
#ifndef MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_NDT_OMP_H_
#define MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_NDT_OMP_H_

#include <pcl/registration/registration.h>
#include <unsupported/Eigen/NonLinearOptimization>
#include "tools/lidars_calibrate/calibrate/include/pclomp/voxel_grid_covariance_omp.h"
namespace crdc {
namespace airi {
namespace pclomp {
...
class NormalDistributionsTransform : public pcl::Registration<PointSource, PointTarget> {
 protected:
    typedef typename pcl::Registration
                                <PointSource, PointTarget>::PointCloudSource PointCloudSource;
    typedef typename PointCloudSource::Ptr PointCloudSourcePtr;
    typedef typename PointCloudSource::ConstPtr PointCloudSourceConstPtr;

    typedef typename pcl::Registration
                                <PointSource, PointTarget>::PointCloudTarget PointCloudTarget;
 ...
 public:
    ...
    typedef boost::shared_ptr
              < const NormalDistributionsTransform<PointSource, PointTarget> > ConstPtr;
 ...
 protected:
    ...
    /** \brief Compute derivatives of probability function w.r.t. the transformation vector.
      ...
      * \param[out] score_gradient the gradient vector of the probability function w.r.t.
      *             the transformation vector
      * \param[out] hessian the hessian matrix of the probability function w.r.t.
      *             the transformation vector
     ...
     *         the transformation vector.
      * \note Equation 6.10, 6.12 and 6.13 [Magnusson 2009].
      * \param[in,out] score_gradient the gradient vector of the probability function w.r.t.
      *                the transformation vector
      * \param[in,out] hessian the hessian matrix of the probability function w.r.t.
      *                the transformation vector
    ...
    /** \brief Compute individual point contirbutions to hessian of probability function w.r.t.
      *        the transformation vector.
      * \note Equation 6.13 [Magnusson 2009].
      * \param[in,out] hessian the hessian matrix of the probability function w.r.t.
      *                the transformation vector
    ...
    /** \brief Compute line search step length and update transform
      *        and probability derivatives using More-Thuente method.
      ...
      * \param[in] step_dir descent direction, \f$ p \f$ in Equation 1.3 (Moore, Thuente 1994)
      *            and \f$ \delta \vec{p} \f$ normalized in Algorithm 2 [Magnusson 2009]
      ...
      *             \f$ f(x + \alpha p) \f$ in Equation 1.3 (Moore, Thuente 1994)
      *             and \f$ score \f$ in Algorithm 2 [Magnusson 2009]
      * \param[in,out] score_gradient gradient of score function w.r.t. transformation vector,
      *                \f$ f'(x + \alpha p) \f$ in Moore-Thuente (1994)
      *                and \f$ \vec{g} \f$ in Algorithm 2 [Magnusson 2009]
      * \param[out] hessian hessian of score function w.r.t. transformation vector,
      *             \f$ f''(x + \alpha p) \f$ in Moore-Thuente (1994)
      *             and \f$ H \f$ in Algorithm 2 [Magnusson 2009]
      * \param[in,out] trans_cloud transformed point cloud,
      *               \f$ X \f$ transformed by \f$ T(\vec{p},\vec{x})
      *               \f$ in Algorithm 2 [Magnusson 2009]
     ...
     *         \f$ I \f$ in More-Thuente (1994)
      * \note Updating Algorithm until some value satifies
      *        \f$ \psi(\alpha_k) \leq 0 \f$ and \f$ \phi'(\alpha_k) \geq 0 \f$
      * and Modified Updating Algorithm from then on [More, Thuente 1994].
      * \param[in,out] a_l first endpoint of interval \f$ I \f$,
      *                 \f$ \alpha_l \f$ in Moore-Thuente (1994)
      * \param[in,out] f_l value at first endpoint, \f$ f_l \f$ in Moore-Thuente (1994),
      *                \f$ \psi(\alpha_l) \f$ for Update Algorithm
      *                 and \f$ \phi(\alpha_l) \f$ for Modified Update Algorithm
      * \param[in,out] g_l derivative at first endpoint, \f$ g_l \f$ in Moore-Thuente (1994),
      *                 \f$ \psi'(\alpha_l) \f$ for Update Algorithm
      *                 and \f$ \phi'(\alpha_l) \f$ for Modified Update Algorithm
      * \param[in,out] a_u second endpoint of interval \f$ I \f$,
      *                 \f$ \alpha_u \f$ in Moore-Thuente (1994)
      * \param[in,out] f_u value at second endpoint, \f$ f_u \f$ in Moore-Thuente (1994),
      *                 \f$ \psi(\alpha_u) \f$ for Update Algorithm
      *                 and \f$ \phi(\alpha_u) \f$ for Modified Update Algorithm
      * \param[in,out] g_u derivative at second endpoint, \f$ g_u \f$ in Moore-Thuente (1994),
      *                 \f$ \psi'(\alpha_u) \f$ for Update Algorithm
      *                 and \f$ \phi'(\alpha_u) \f$ for Modified Update Algorithm
      ...
      *            \f$ \psi(\alpha_t) \f$ for Update Algorithm
      *            and \f$ \phi(\alpha_t) \f$ for Modified Update Algorithm
      * \param[in] g_t derivative at trial value, \f$ g_t \f$ in Moore-Thuente (1994),
      *            \f$ \psi'(\alpha_t) \f$ for Update Algorithm
      *            and \f$ \phi'(\alpha_t) \f$ for Modified Update Algorithm
    ...
    /** \brief Select new trial value for More-Thuente method.
      ...
      * is used for \f$ f_k \f$ and \f$ g_k \f$
      * until some value satifies the test \f$ \psi(\alpha_k) \leq 0 \f$
      * and \f$ \phi'(\alpha_k) \geq 0 \f$
      * then \f$ \phi(\alpha_k) \f$ is used from then on.
      * \note Interpolation Minimizer equations from Optimization Theory and Methods:
      *       Nonlinear Programming By Wenyu Sun, Ya-xiang Yuan (89-100).
      ...
      * \param[in] a_u second endpoint of interval \f$ I \f$, \f$ \alpha_u \f$
    ...
    /** \brief The second order derivative of the transformation of 
     * a point w.r.t. the transform vector, \f$ H_E \f$ in Equation 6.20 [Magnusson 2009]. */
    //      Eigen::Matrix<double, 18, 6> point_hessian_;
    int num_threads_;
...
#endif  // MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_NDT_OMP_H_
