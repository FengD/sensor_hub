...
#ifndef MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_VOXEL_GRID_COVARIANCE_OMP_H_
#define MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_VOXEL_GRID_COVARIANCE_OMP_H_
...
class VoxelGridCovariance : public pcl::VoxelGrid<PointT> {
 ...
 public:
  ...
  struct Leaf {
    /** \brief Constructor.
     * Sets \ref nr_points, \ref icov_, \ref mean_ and \ref evals_ to 0
     * and \ref cov_ and \ref evecs_ to the identity matrix
     */
    Leaf()
        : nr_points(0),
 ...
 public:
   ...
   * \param[in] min_points_per_voxel the minimum number of points
   *            for required for a voxel to be used
   */
  inline void setMinPointPerVoxel(int min_points_per_voxel) {
    if (min_points_per_voxel > 2) {
      min_points_per_voxel_ = min_points_per_voxel;
    } else {
      PCL_WARN("%s: requires at least 3 points, setting Min Point per Voxel to 3 ",
  ...
  /** \brief Set the minimum allowable ratio between eigenvalues
  ...
  /** \brief Get the minimum allowable ratio between eigenvalues
   *         to prevent singular covariance matrices.
   ...
   * \param[out] output cloud containing centroids of
   *             voxels containing a sufficient number of points
 ...
 protected:
  ...
  /** \brief Minimum allowable ratio between eigenvalues
   * to prevent singular covariance matrices. */
  ...
  /** \brief Point cloud containing centroids of voxels containing
   * atleast minimum number of points. */
  PointCloudPtr voxel_centroids_;

  /** \brief Indices of leaf structurs associated with each point
   * in \ref voxel_centroids_ (used for searching). */
...
#endif  // MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_VOXEL_GRID_COVARIANCE_OMP_H_
