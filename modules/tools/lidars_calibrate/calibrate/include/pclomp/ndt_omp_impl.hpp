...
#include <algorithm>
#include <vector>
#include "tools/lidars_calibrate/calibrate/include/pclomp/ndt_omp.h"
#ifndef MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_NDT_OMP_IMPL_HPP_
#define MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_NDT_OMP_IMPL_HPP_
namespace crdc {
namespace airi {
...
pclomp::NormalDistributionsTransform<PointSource, PointTarget>::NormalDistributionsTransform()
  ...
  , gauss_d3_()
  , trans_probability_()
  , j_ang_a_(), j_ang_b_(), j_ang_c_(), j_ang_d_(), j_ang_e_(), j_ang_f_()
  , j_ang_g_(), j_ang_h_(), h_ang_a2_(), h_ang_a3_(), h_ang_b2_(), h_ang_b3_()
  , h_ang_c2_(), h_ang_c3_(), h_ang_d1_(), h_ang_d2_(), h_ang_d3_(), h_ang_e1_()
...
pclomp::NormalDistributionsTransform<PointSource, PointTarget>::computeDerivatives(
  ...
  std::vector<Eigen::Matrix<double, 6, 1>,
            Eigen::aligned_allocator<Eigen::Matrix<double, 6, 1>>> score_gradients(num_threads_);
  std::vector<Eigen::Matrix<double, 6, 6>,
            Eigen::aligned_allocator<Eigen::Matrix<double, 6, 6>>> hessians(num_threads_);
  ...
  unsigned int pointsSize = input_->points.size();
    // Update gradient and hessian for each point, line 17 in Algorithm 2 [Magnusson 2009]
  #pragma omp parallel for num_threads(num_threads_) schedule(guided, 8)

  for (unsigned int idx = 0; idx < pointsSize; idx++) {
    ...
    for (typename std::vector<TargetGridLeafConstPtr>::iterator
      ...
      // Compute derivative of transform function w.r.t.
      // transform vector, J_E and H_E in Equations 6.18 and 6.20 [Magnusson 2009]
...
pclomp::NormalDistributionsTransform<PointSource, PointTarget>::computeAngleDerivatives(
  ...
  j_ang.setZero();
  j_ang.row(0).noalias() = Eigen::Vector4f((-sx * sz + cx * sy * cz),
                                          (-sx * cz - cx * sy * sz), (-cx * cy), 0.0f);
  j_ang.row(1).noalias() = Eigen::Vector4f((cx * sz + sx * sy * cz),
                                          (cx * cz - sx * sy * sz), (-sx * cy), 0.0f);
  ...
  if (compute_hessian) {
    ...
    h_ang.row(0).noalias() = Eigen::Vector4f((-cx * sz - sx * sy * cz),
                                            (-cx * cz + sx * sy * sz), sx * cy, 0.0f);      // a2
    h_ang.row(1).noalias() = Eigen::Vector4f((-sx * sz + cx * sy * cz),
                                            (-cx * sy * sz - sx * cz), (-cx * cy), 0.0f);   // a3

    h_ang.row(2).noalias() = Eigen::Vector4f((cx * cy * cz),
                                            (-cx * cy * sz), (cx * sy), 0.0f);              // b2
    h_ang.row(3).noalias() = Eigen::Vector4f((sx * cy * cz),
                                            (-sx * cy * sz), (sx * sy), 0.0f);              // b3

    h_ang.row(4).noalias() = Eigen::Vector4f((-sx * cz - cx * sy * sz),
                                            (sx * sz - cx * sy * cz), 0, 0.0f);             // c2
    h_ang.row(5).noalias() = Eigen::Vector4f((cx * cz - sx * sy * sz),
                                            (-sx * sy * cz - cx * sz), 0, 0.0f);            // c3

    h_ang.row(6).noalias() = Eigen::Vector4f((-cy * cz), (cy * sz), (sy), 0.0f);            // d1
    h_ang.row(7).noalias() = Eigen::Vector4f((-sx * sy * cz),
                                            (sx * sy * sz), (sx * cy), 0.0f);               // d2
    h_ang.row(8).noalias() = Eigen::Vector4f((cx * sy * cz),
                                            (-cx * sy * sz), (-cx * cy), 0.0f);             // d3
    ...
    h_ang.row(13).noalias() = Eigen::Vector4f((-cx * sz - sx * sy * cz),
                                              (-cx * cz + sx * sy * sz), 0, 0.0f);          // f2
    h_ang.row(14).noalias() = Eigen::Vector4f((-sx * sz + cx * sy * cz),
                                              (-cx * sy * sz - sx * cz), 0, 0.0f);          // f3
  }
}
...
pclomp::NormalDistributionsTransform<PointSource, PointTarget>::computePointDerivatives(
  ...
  Eigen::Vector4f x4(x[0], x[1], x[2], 0.0f);
    ...
    // Derivative w.r.t. ith element of transform vector corresponds to
...
pclomp::NormalDistributionsTransform<PointSource, PointTarget>::computePointDerivatives(
  ...
  if (compute_hessian) {
    ...
    // Calculate second derivative of Transformation Equation 6.17 w.r.t. transform vector p.
    // Derivative w.r.t. ith and jth elements of transform vector corresponds
    // to the 3x1 block matrix starting at (3i,j), Equation 6.20 and 6.21 [Magnusson 2009]
...
pclomp::NormalDistributionsTransform<PointSource, PointTarget>::computeHessian(
  ...
  hessian.setZero();
    ...
    // Precompute Angular Derivatives unessisary because only used
    // after regular derivative calculation
  unsigned int pointsSize = input_->points.size();
...
template<typename PointSource, typename PointTarget> double
pclomp::NormalDistributionsTransform<PointSource, PointTarget>::computeStepLengthMT(const
  ...
  double g_u = auxilaryFunction_dPsiMT(d_phi_0, d_phi_0, mu);
    ...
    // Check used to allow More-Thuente step length calculation to be skipped
    // by making step_min == step_max
  ...
  final_transformation_ = (Eigen::Translation<float, 3>(static_cast<float>(x_t(0)),
                              static_cast<float>(x_t(1)), static_cast<float>(x_t(2))) *
  ...
  // but testing showed that most step calculations use the
  // initial step suggestion
  // and recalculation the reusable portions of the hessian would intail more computation time.
  ...
  // Iterate until max number of iterations, interval convergance or
  // a value satisfies the sufficient decrease, Equation 1.1, and curvature condition,
  // Equation 1.2 [More, Thuente 1994]
  while (!interval_converged && step_iterations < max_step_iterations && !(psi_t <= 0
        /*Sufficient Decrease*/ && d_phi_t <= -nu * d_phi_0 /*Curvature Condition*/)) {
...
double pclomp::NormalDistributionsTransform<PointSource, PointTarget>::calculateScore(
  ...
  double score = 0;
  unsigned int pointsSize = trans_cloud.points.size();
  for (unsigned int idx = 0; idx < pointsSize; idx++) {
...
#endif  // MODULES_TOOLS_LIDARS_CALIBRATE_CALIBRATE_INCLUDE_PCLOMP_NDT_OMP_IMPL_HPP_
