...
template <typename T>
void ImageDecode::decode_h264frame(std::string& image_name, T& decode_msg, bool& deflag) {
  if (!std::getenv("TYPE")) {
...
template <typename T>
void ImageDecode::decode_cvframe(std::string topic_name, cv::Mat& image_jpeg_,
...
void ImageDecode::decode(rclcpp::SerializedMessage& extracted_serialized_msg,
...
void ImageDecode::imagepublish(sensor_msg::msg::Image& image_msg_, cv::Mat& process_img_,
...
void ImageDecode::imagepublish(sensor_msgs::msg::CompressedImage& image_msg_, cv::Mat& process_img_,
...
void ImageDecode::get_img_ld(cv::Mat& img_ld) {
  img_ld = process_img_ld_;
  process_img_ld_.release();
}

void ImageDecode::get_img_fs(cv::Mat& img_fs) {
  img_fs = process_img_fs_;
  process_img_fs_.release();
}

void ImageDecode::clear_image_map() {
  fs_mask_map_.clear();
  ld_mask_map_.clear();
  fs_image_map_.clear();
  ld_image_map_.clear();
