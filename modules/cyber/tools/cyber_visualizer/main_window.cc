...
#include "cyber/tools/cyber_visualizer/fixedaspectratiowidget.h"
#include "cyber/tools/cyber_visualizer/grid.h"
#include "cyber/tools/cyber_visualizer/main_window.h"
#include "cyber/tools/cyber_visualizer/pointcloud.h"
#include "cyber/tools/cyber_visualizer/radarpoints.h"
#include "cyber/tools/cyber_visualizer/ui_main_window.h"
#include "cyber/tools/cyber_visualizer/video_images_dialog.h"
...
struct MainWindow::VideoImgProxy {
  ...
  union {
    CyberChannReader<crdc::airi::Image2>* image_reader_;
...
void MainWindow::PointCloudReaderCallback(
  ...
  pointcloud_reader_mutex_.lock();
  pointcloud_reader_mutex_.unlock();
  PointCloud* pc = new PointCloud(pdata->width() * pdata->height(), 4, pointcloud_shader_);
  if (pc) {
  ...
  if (b) {
    ...
    if (!pointcloud_channel_Reader_) {
      pointcloud_channel_Reader_ =
          new CyberChannReader<crdc::airi::PointCloud2>();
      ...
      auto pointCallback =
          [this](const std::shared_ptr<crdc::airi::PointCloud2>& pdata) {
    ...
    if (!theVideoImg->image_reader_) {
      if (theVideoImg->isCompressedImage_) {
        ...
        theVideoImg->image_reader_ =
            new CyberChannReader<crdc::airi::Image2>();
      }
      ...
      if (theVideoImg->isCompressedImage_) {
        ...
        auto videoCallback =
            [this, theVideoImg](
                const std::shared_ptr<crdc::airi::Image2>& pdata) {
