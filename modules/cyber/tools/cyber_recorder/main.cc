...
const char RECORD_OPTIONS[] = "o:t:x:ac:i:m:h";
  ...
  for (char option : options) {
    switch (option) {
      ...
      case 'o':
        ...
        break;
      case 't':
        std::cout << "\t-t, --output path <dir>\t\t\toutput record file path (file name is timestamp)"
                  << std::endl;
        break;
      case 'x':
        std::cout << "\t-x, --output prefix <str>\t\toutput record file name prefix (file name is prefix+timestamp)"
                  << std::endl;
        break;
      case 'a':
...
int main(int argc, char** argv) {
  ...
  int long_index = 0;
  const std::string short_opts = "f:c:k:o:t:x:alr:b:e:s:d:p:i:m:h";
  static const struct option long_opts[] = {
      ...
      {"path", required_argument, nullptr, 't'},
      {"output-prefix", required_argument, nullptr, 'x'},
  ...
  std::vector<std::string> opt_outpath_vec;
  std::vector<std::string> opt_outprefix_vec;
  std::vector<std::string> opt_white_channels;
  std::vector<std::string> opt_black_channels;
  bool opt_use_time_name_file = false;
  bool opt_all = false;
  bool opt_loop = false;
  float opt_rate = 1.0f;
  ...
  do {
    ...
    switch (opt) {
      ...
      case 'o':
        ...
        break;
      case 't':
        opt_outpath_vec.push_back(std::string(optarg));
        opt_use_time_name_file = true;
        break;
      case 'x':
        opt_outprefix_vec.push_back(std::string(optarg));
        break;
      case 'a':
        opt_all = true;
        break;
  ...
  if (command == "play") {
    ...
    if (opt_outpath_vec.size() > 1) {
      std::cout << "TOO many output file path option (-t)." << std::endl;
      return -1;
    }
    if (opt_outprefix_vec.size() > 1) {
      std::cout << "TOO many output file prefix option (-x)." << std::endl;
      return -1;
    }
    if (opt_output_vec.size() > 0 && opt_outpath_vec.size() > 0) {
      std::cout << "Output file and output file path can not be specified simultaneously (-o/-t)." << std::endl;
      return -1;
    }
    if (opt_output_vec.size() > 0 && opt_outprefix_vec.size() > 0) {
      std::cout << "Output file and output file prefix can not be specified simultaneously (-o/-x)." << std::endl;
      return -1;
    }
    if (opt_output_vec.empty() && opt_outpath_vec.empty()) {
      std::string default_output_file =
          UnixSecondsToString(time(nullptr), "%Y%m%d%H%M%S") + ".record";
      opt_output_vec.push_back(default_output_file);
    }
    if (opt_output_vec.empty() && opt_outpath_vec.size() > 0) {
      std::string default_output_file;
      if (!opt_outprefix_vec.empty()) {
        default_output_file =
            opt_outpath_vec[0] + "/" + opt_outprefix_vec[0] +
            UnixSecondsToString(time(nullptr), "%Y-%m-%d-%H-%M-%S") + "-000.record";
      } else {
        default_output_file =
            opt_outpath_vec[0] + "/" +
            UnixSecondsToString(time(nullptr), "%Y-%m-%d-%H-%M-%S") + "-000.record";
    ...
    recorder->SetUseTimeNameFile(opt_use_time_name_file);
    if (!opt_outpath_vec.empty()) {
      recorder->SetOutPath(opt_outpath_vec[0]);
    }
    if (!opt_outprefix_vec.empty()) {
      recorder->SetOutPrefix(opt_outprefix_vec[0]);
    }
